...
def test_get_descendant_roles(flake: PyflakeClient):
    ...
    ### Act ###
    hierarchy: RoleDescendantsEntity = flake.describe(RoleDescendantsDescribable("ACCOUNTADMIN"), RoleDescendantsEntity)

    sec_admin = next((r for r in hierarchy.descendant_roles if r.role_name == "SECURITYADMIN"), None)
    sys_admin = next((r for r in hierarchy.descendant_roles if r.role_name == "SYSADMIN"), None)

    ### Assert ###
    assert hierarchy.name == "ACCOUNTADMIN"
    assert sec_admin is not None
    assert sec_admin.distance_from_source == 0  # SECURITYADMIN is a direct child of ACCOUNTADMIN
    assert sec_admin.role_name == "SECURITYADMIN"
    assert sec_admin.parent_role_name == "ACCOUNTADMIN"

    assert sys_admin is not None
    assert sys_admin.distance_from_source == 0  # SYSADMIN is a direct child of ACCOUNTADMIN
