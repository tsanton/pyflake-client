...
def test_describe_grant_for_non_existing_role(flake: PyflakeClient):
    ...
    ### Act ###
    grants = flake.describe_async(describable=GrantDescribable(principal=RoleDescribable(name="NON_EXISTING_ROLE"))).deserialize_many(GrantEntity)
...
def test_role_account_grants(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    try:
        ...
        ### Act ###
        grants = flake.describe_async(describable=GrantDescribable(principal=RoleDescribable(name=role.name))).deserialize_many(GrantEntity)
