...
from pyflake_client.models.assets.table_columns import Date
from pyflake_client.models.describables.table import Table as TableDescribable
from pyflake_client.models.entities.column import Date as DateEntity
from pyflake_client.models.entities.table import Table as TableEntity


def test_table_date(flake: PyflakeClient, assets_queue: queue.LifoQueue, rand_str: str, comment: str):
    ...
    try:
        ...
        assert isinstance(c, DateEntity)
        assert c.type == "DATE"
...
def test_table_date_primary_key(flake: PyflakeClient, assets_queue: queue.LifoQueue, rand_str: str, comment: str):
    database = DatabaseAsset(f"PYFLAKE_CLIENT_TEST_DB_{rand_str}", comment, owner=RoleAsset("SYSADMIN"))
    schema = Schema(
        db_name=database.db_name,
        schema_name="TEST_SCHEMA",
    ...
    columns = [
        Date(name="DATE_COLUMN", primary_key=True),
    ]
    table = TableAsset(
        db_name=database.db_name,
        schema_name=schema.schema_name,
