...
def test_create_role_ascendants(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    ### Arrange ###
    child1_role = Role("IGT_CHILD1_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
    child2_role = Role("IGT_CHILD2_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
    parent_role = Role("IGT_PARENT_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
    ...
    grandparent1_role = Role("IGT_GRANDPARENT1_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
    grandparent2_role = Role("IGT_GRANDPARENT2_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
    ...
    great_grandparent_role = Role("IGT_GREAT_GRANDPARENT_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
...
def test_broken_role_ascendants(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    parent_child1_relationship = RoleRelationship(child1_role.name, parent_role.name)
    parent_child2_relationship = RoleRelationship(child2_role.name, parent_role.name)
    grandparent_role = Role("IGT_GRANDPARENT_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
