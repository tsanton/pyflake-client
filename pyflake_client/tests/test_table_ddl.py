...
def test_create_simple_table_ddl(db_asset_fixture: Database):
    ...
    assert (
        definition
        == """create or replace table IGT_DEMO.S1.TEST(ID INTEGER IDENTITY (1,1) NOT NULL)"""
    )
...
def test_create_complex_table_ddl(db_asset_fixture: Database):
    ...
    assert (
        definition
        == "create or replace table IGT_DEMO.S1.TEST(ID INTEGER IDENTITY (1,1) NOT NULL, VARCHAR_NO_DEFAULT VARCHAR(16777216) NOT NULL, VARCHAR_DEFAULT VARCHAR(16777216) NOT NULL DEFAULT 'YES')"
    )
...
def test_create_simple_table_with_default_date_ddl(db_asset_fixture: Database):
    ...
    assert (
        definition
        == """create or replace table IGT_DEMO.S1.TEST(ID INTEGER IDENTITY (1,1) NOT NULL, SOME_DATE DATE NOT NULL DEFAULT '2000-01-01'::date)"""
    )
