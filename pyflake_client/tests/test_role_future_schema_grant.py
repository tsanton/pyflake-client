...
def test_grant_role_future_schema_privilege(
    ...
    privilege = GrantAsset(RoleSchemaFutureGrant(role.name, db_asset_fixture.db_name, schema.schema_name, ObjectType.TABLE), ["SELECT"])
    ...
    try:
        ...
        assert priv.name == f"{db_asset_fixture.db_name}.{schema.schema_name}.<{ObjectType.TABLE}>"
...
def test_grant_role_schema_privileges(
    ...
    schema = AssetsSchema(database=db_asset_fixture, schema_name="IGT_SCHEMA", comment=f"pyflake_client_TEST_{uuid.uuid4()}")
    role = AssetsRole("IGT_CREATE_ROLE", AssetsRole("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}")
    p1 = GrantAsset(RoleSchemaFutureGrant(role.name, db_asset_fixture.db_name, schema.schema_name, ObjectType.TABLE), ["SELECT", "UPDATE"])
    p2 = GrantAsset(RoleSchemaFutureGrant(role.name, db_asset_fixture.db_name, schema.schema_name, ObjectType.VIEW),  ["SELECT", "REFERENCES"])
    ...
    try:
        ...
        table_schema_scope = f"{db_asset_fixture.db_name}.{schema.schema_name}.<{ObjectType.TABLE}>"
        ...
        view_schema_scope = f"{db_asset_fixture.db_name}.{schema.schema_name}.<{ObjectType.VIEW}>"
