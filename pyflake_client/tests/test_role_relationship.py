...
from pyflake_client.models.assets.role_relationship import (
    RoleRelationship as RoleRelationshipAsset,
)
from pyflake_client.models.assets.role import Role
from pyflake_client.models.describables.role_relationship import (
    RoleRelationship as RoleRelationshipDescribable,
)
from pyflake_client.models.entities.role_relationship import (
    RoleRelationship as RoleRelationshipEntity,
)
...
def test_get_role_relationship(flake: PyflakeClient):
    ...
    ### Act ###
    role: RoleRelationshipEntity = flake.describe_one(
        RoleRelationshipDescribable("SYSADMIN", "ACCOUNTADMIN"), RoleRelationshipEntity
    )
...
def test_create_role_relationship(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    child_role: Role = Role(
        "IGT_CHILD_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}"
    )
    ...
    try:
        ...
        flake.register_asset(
            RoleRelationshipAsset(child_role.name, parent_role.name), assets_queue
        )

        ### Act ###
        rel: RoleRelationshipEntity = flake.describe_one(
            RoleRelationshipDescribable(child_role.name, parent_role.name),
...
def test_delete_role_relationship(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    relationship_describable = RoleRelationshipDescribable(
    ...
    try:
        ...
        new_rel: RoleRelationshipEntity = flake.describe_one(
            relationship_describable, RoleRelationshipEntity
        )

        ### Act ###
        flake.delete_asset(relationship)
        del_rel: RoleRelationshipEntity = flake.describe_one(
            relationship_describable, RoleRelationshipEntity
        )
...
def test_get_non_existing_role_relationship(flake: PyflakeClient):
    ...
    ### Act ###
    role: RoleRelationshipEntity = flake.describe_one(
        RoleRelationshipDescribable("ACCOUNTADMIN", "SYSADMIN"), RoleRelationshipEntity
    )
