...
def show_table_description_py(snowpark_session, db_name_py:str, schema_name_py:str, table_name_py:str):
    ...
    table['tags'] = []
    tag_query: str = f""SELECT * from table({self.database_name}.INFORMATION_SCHEMA.TAG_REFERENCES('{self.database_name}.{self.schema_name}.{self.name}', 'TABLE'));""
    ...
    column_tags_query:str = f""SELECT * from table({self.database_name}.INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('{self.database_name}.{self.schema_name}.{self.name}', 'TABLE'));""
    ...
    return table
$$
call show_table_description('{self.database_name}', '{self.schema_name}', '{self.name}')";
        return query;
        """
