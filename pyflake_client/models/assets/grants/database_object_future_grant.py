...
class DatabaseObjectFutureGrant(ISnowflakeGrantAsset):
    ...
    def get_grant_statement(self, principal: ISnowflakePrincipal, privileges: List[Privilege]) -> str:
        ...
        if isinstance(principal, RoleAsset):
            return f"GRANT {privs} ON FUTURE {self.grant_target.pluralize()} IN DATABASE {self.database_name} TO ROLE {principal.get_identifier()}"
        if isinstance(principal, DatabaseRoleAsset):
            return f"GRANT {privs} ON FUTURE {self.grant_target.pluralize()} IN DATABASE {self.database_name} TO DATABASE ROLE {principal.get_identifier()}"
    ...
    def get_revoke_statement(self, principal: ISnowflakePrincipal, privileges: List[Privilege]) -> str:
        privs = f"{', '.join(p.value for p in privileges)}"
        if isinstance(principal, RoleAsset):
            return f"REVOKE {privs} ON FUTURE {self.grant_target.pluralize()} IN DATABASE {self.database_name} FROM ROLE {principal.get_identifier()} CASCADE"
        if isinstance(principal, DatabaseRoleAsset):
            return f"REVOKE {privs} ON FUTURE {self.grant_target.pluralize()} IN DATABASE {self.database_name} FROM DATABASE ROLE {principal.get_identifier()} CASCADE"
