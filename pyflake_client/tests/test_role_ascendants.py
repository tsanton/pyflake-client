...
from pyflake_client.models.entities.role_ascendants import (
    RoleAscendants as RoleAscendantsEntity,
)
from pyflake_client.models.describables.role_ascendants import (
    RoleAscendants as RoleAscendantsDescribable,
)
...
def test_get_role_ascendants(flake: PyflakeClient):
    ...
    hierarchy: RoleAscendantsEntity = flake.describe_one(
        RoleAscendantsDescribable("USERADMIN"), RoleAscendantsEntity
    )

    sec_admin = next(
        (r for r in hierarchy.ascendant_roles if r.parent_role_name == "SECURITYADMIN"),
        None,
    )
    acc_admin = next(
        (r for r in hierarchy.ascendant_roles if r.parent_role_name == "ACCOUNTADMIN"),
        None,
    )
    sys_admin = next(
        (r for r in hierarchy.ascendant_roles if r.parent_role_name == "SYSADMIN"), None
    )
    ...
    )  # SECURITYADMIN is a direct parent of USERADMIN
    assert sec_admin.role_name == "USERADMIN"
    assert sec_admin.parent_role_name == "SECURITYADMIN"
    assert acc_admin is not None
    assert (
        acc_admin.distance_from_source == 1
    )  # ACCOUNTADMIN is a grantparent of USERADMIN
...
def test_create_role_ascendants(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    ### Arrange ###
    child1_role = Role(
        "IGT_CHILD1_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}"
    )
    child2_role = Role(
        "IGT_CHILD2_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}"
    )
    parent_role = Role(
        "IGT_PARENT_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}"
    )
    ...
    grandparent1_role = Role(
        "IGT_GRANDPARENT1_ROLE",
        Role("USERADMIN"),
        f"pyflake_client_TEST_{uuid.uuid4()}",
    )
    grandparent2_role = Role(
        "IGT_GRANDPARENT2_ROLE",
        Role("USERADMIN"),
        f"pyflake_client_TEST_{uuid.uuid4()}",
    )
    grandparent1_parent_relationship = RoleRelationship(
        parent_role.name, grandparent1_role.name
    )
    grandparent2_parent_relationship = RoleRelationship(
        parent_role.name, grandparent2_role.name
    )
    great_grandparent_role = Role(
        "IGT_GREAT_GRANDPARENT_ROLE",
        Role("USERADMIN"),
        f"pyflake_client_TEST_{uuid.uuid4()}",
    )
    great_grandparent_grandparent_relationship = RoleRelationship(
        grandparent1_role.name, great_grandparent_role.name
    )
    sysadmin_great_grandparent_relationship = RoleRelationship(
        great_grandparent_role.name, "SYSADMIN"
    )
    try:
        ...
        ### Act ###
        hierarchy: RoleAscendantsEntity = flake.describe_one(
            RoleAscendantsDescribable("IGT_CHILD1_ROLE"), RoleAscendantsEntity
        )
        parent = next(
            (
                r
                for r in hierarchy.ascendant_roles
                if r.parent_role_name == parent_role.name
            ),
            None,
        )
        grandparent1 = next(
            (
                r
                for r in hierarchy.ascendant_roles
                if r.parent_role_name == grandparent1_role.name
            ),
            None,
        )
        grandparent2 = next(
            (
                r
                for r in hierarchy.ascendant_roles
                if r.parent_role_name == grandparent2_role.name
            ),
            None,
        )
        great_grandparent = next(
            (
                r
                for r in hierarchy.ascendant_roles
                if r.parent_role_name == great_grandparent_role.name
            ),
            None,
        )
        sys_admin = next(
            (r for r in hierarchy.ascendant_roles if r.parent_role_name == "SYSADMIN"),
            None,
        )
        acc_admin = next(
            (
                r
                for r in hierarchy.ascendant_roles
                if r.parent_role_name == "ACCOUNTADMIN"
            ),
            None,
        )
...
def test_broken_role_ascendants(flake: PyflakeClient, assets_queue: queue.LifoQueue):
    ...
    parent_child1_relationship = RoleRelationship(child1_role.name, parent_role.name)
    parent_child2_relationship = RoleRelationship(child2_role.name, parent_role.name)
    grandparent_role = Role(
        "IGT_GRANDPARENT_ROLE", Role("USERADMIN"), f"pyflake_client_TEST_{uuid.uuid4()}"
    )
    grandparent_parent_relationship = RoleRelationship(
        parent_role.name, grandparent_role.name
    )
    great_grandparent_role = Role(
        "IGT_GREAT_GRANDPARENT_ROLE",
        Role("USERADMIN"),
    ...
    try:
        ...
        grandparent = next(
            (
                r
                for r in hierarchy.ascendant_roles
                if r.parent_role_name == grandparent_role.name
            ),
            None,
        )
        great_grandparent = next(
